...
%gDesiredIni = [ 10 10 10 10 10 10 10 10 10];
...
%cd (folderAbsor); gDesiredIni
%load ("FiInfAbsorb.mat");
%gDesiredIni = 20*log10(absorbData1(1,:));
%gDesiredIni = [0.59, 0.21, 0.33, 0.23, 0.26, 0.37, 0.46, 0.42, 0.67];


% gDesiredIni=[0.39  0.95  0.58  0.82  0.71  0.51  0.45  0.56  0.28]; %%1
%gDesiredIni=[0.15  0.90  0.33  0.67  0.50  0.26  0.20  0.32  0.08]; %%2
% gDesiredIni=[0.06  0.85  0.19  0.55  0.35  0.13  0.09  0.18  0.02]; %%3
% 
%gDesiredIni=[0.71  0.71  0.71  0.71  0.71  0.71  0.71  0.71 0.71]; %%1
%gDesiredIni=[0.50  0.50  0.50  0.50  0.50  0.50  0.50  0.50 0.50]; %%2
%gDesiredIni=[0.35  0.35  0.35  0.35  0.35  0.35  0.35  0.35 0.35]; %%3
%gDesiredIni=[0.25  0.25  0.25  0.25  0.25  0.25  0.25  0.25 0.25]; %%3
gDesiredIni=[ 0.79 0.28 0.06 0.19 0.18 0.36 0.60 0.43 0.84]; %%3
gDesiredIni = 1-gDesiredIni.^2;
...
[b0,b1,b2,a0,a1,a2] = calculateShelCoefVal.LowP_Valimak (Fs, F0*sqrt(2), Qs, g(1));
a(1,:)= [a0, a1, a2];
b(1,:)= [b0, b1, b2];
...
for i=2:NF-1
    ...
    [b0,b1,b2,a0,a1,a2] = calculateShelCoefVal.BandP_Valimak (Fs, Fc, Qbp, g(i));
    a(i,:)= [a0, a1, a2];
    b(i,:)= [b0, b1, b2];
...
[b0,b1,b2,a0,a1, a2] = calculateShelCoefVal.HighP_Valimak (Fs, Fc/sqrt(2), Qs, g(NF));
a(NF,:)= [a0, a1, a2];
b(NF,:)= [b0, b1, b2];
