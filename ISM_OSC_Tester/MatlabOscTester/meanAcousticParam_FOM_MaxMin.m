...
% 2-> rooms, 4 --> Tmix
C80_dist_max = zeros (2,4);  C80_dist_max_current = zeros(1,2);
EEy_dist_max = zeros (2,4);  EEy_dist_max_current = zeros(1,2);
T20_dist_max = zeros (2,4);  T20_dist_max_current = zeros(1,2);
EDT_dist_max = zeros (2,4);  EDT_dist_max_current = zeros(1,2);
...
for j=1:length(eFolders)
    ...
    if ~(strcmp(tMix,'40'))
        ...
        maxPar = max (C80_sJun_dist,[],"all");
        if C80_dist_max_current(2) < maxPar
            C80_dist_max (2,j )  = maxPar;     C80_dist_max_current(2) = maxPar;
        end

        maxPar = max (EEy_sJun_dist,[],"all");
        if EEy_dist_max_current(2) < maxPar
            EEy_dist_max (2,j )  = maxPar;     EEy_dist_max_current(2) = maxPar;
        end

        maxPar = max (T20_sJun_dist,[],"all");
        if T20_dist_max_current(2) < maxPar
            T20_dist_max (2,j )  = maxPar;     T20_dist_max_current(2) = maxPar;
        end

        maxPar = max (EDT_sJun_dist,[],"all");
        if EDT_dist_max_current(2) < maxPar
            EDT_dist_max (2,j )  = maxPar;     EDT_dist_max_current(2) = maxPar;
        end
    ...
    if ~(strcmp(tMix,'34'))
        ...
        maxPar = max (C80_A108_dist,[],"all");
        if C80_dist_max_current(1) < maxPar
            C80_dist_max (1,j )  = maxPar;       C80_dist_max_current(1) = maxPar;
        end

        maxPar = max (EEy_A108_dist,[],"all");
        if EEy_dist_max_current(1) < maxPar
            EEy_dist_max (1,j )  = maxPar;       EEy_dist_max_current(1) = maxPar;
        end
        
        maxPar = max (T20_A108_dist,[],"all");
        if T20_dist_max_current(1) < maxPar
            T20_dist_max (1,j )  = maxPar;       T20_dist_max_current(1) = maxPar;
        end

        maxPar = max (EDT_A108_dist,[],"all");
        if EDT_dist_max_current(1) < maxPar
            EDT_dist_max (1,j )  = maxPar;       EDT_dist_max_current(1) = maxPar;
        end
        ...
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        set(fig_dist{j+4},'Units','normalized');
        set(fig_dist{j+4},'Position',[0.180729166666667,0.159259259259259,0.708854166666667,0.348148148148148]);
        room = 'A108';
        title(tcl,['Room ' room  ' Tmix ' tMix]);
        saveas(fig_dist{j+4},fullfile(path_save,[room '_dist_params_Tmix_' tMix '.fig']));
        saveas(fig_dist{j+4},fullfile(path_save,[room '_dist_params_Tmix_' tMix '.png']));
    end
...
for j=1:length(eFolders)
    ...
    if ~(strcmp(tMix,'40'))
        ...
        h = heatmap(xvalues, yvalues, T20_sJun_dist([1:4],:)'); title (['T20 Distance to ref']); 
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = T20_dist_max_current (2);
        h.ColorLimits = [0 vMax];
        
        nexttile(2); 
        h = heatmap(xvalues, yvalues, EDT_sJun_dist([1:4],:)'); title (['EDT Distance to ref']); 
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = EDT_dist_max_current (2);
        h.ColorLimits = [0 vMax];
        
        nexttile(3);
        h= heatmap(xvalues, yvalues, C80_sJun_dist([1:4],:)'); title (['C80 Distance to ref']);  
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = C80_dist_max_current (2);
        h.ColorLimits = [0 vMax];
        
        nexttile(4);
        h = heatmap(xvalues, yvalues, EEy_sJun_dist'); title (['EEy Distance to ref']); 
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = EEy_dist_max_current (2);
        h.ColorLimits = [0 vMax];
    ...
    if ~(strcmp(tMix,'34'))
        ...
        h = heatmap(xvalues, yvalues, T20_A108_dist([1:4],:)'); title (['T20 Distance to ref']); 
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = T20_dist_max_current (1);
        h.ColorLimits = [0 vMax];
        
        nexttile(2); 
        h = heatmap(xvalues, yvalues, EDT_A108_dist([1:4],:)'); title (['EDT Distance to ref']); 
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = EDT_dist_max_current (1);
        h.ColorLimits = [0 vMax];
        
        nexttile(3);
        h= heatmap(xvalues, yvalues, C80_A108_dist([1:4],:)'); title (['C80 Distance to ref']);  
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = C80_dist_max_current (1);
        h.ColorLimits = [0 vMax];
        
        nexttile(4);
        h = heatmap(xvalues, yvalues, EEy_A108_dist'); title (['EEy Distance to ref']); 
        h.ColorMethod = 'min'; h.Colormap = sky(10); %h.ColorScaling = 'scaledcolumns';
        vMax = EEy_dist_max_current (1);
        h.ColorLimits = [0 vMax];
